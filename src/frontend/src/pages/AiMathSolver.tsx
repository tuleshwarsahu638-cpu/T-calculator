...
export function parseAndSolve(input: string): Solution | null {
  ...
  // Age word problems — "father's age is X times son's age, sum of their
  // ages is Y" (classic Class 7-9 linear-equation pattern)
  const ageSumMatch = wordClean.match(
    /(?:father|mother|father's|mother's).*?(\d+(?:\.\d+)?)\s*times.*?(?:son|daughter|son's|daughter's).*?(?:sum|total).*?(?:age|ages).*?(?:is|=)\s*(\d+(?:\.\d+)?)/,
  );
  if (ageSumMatch) {
    const multiple = Number.parseFloat(ageSumMatch[1]);
    const sum = Number.parseFloat(ageSumMatch[2]);
    // son + multiple*son = sum  =>  son = sum / (multiple + 1)
    const childAge = sum / (multiple + 1);
    const parentAge = sum - childAge;
    steps.push({
      description: "Identified: Age problem (ratio + sum)",
      value: `Let child's age = x, parent's age = ${multiple}x, sum = ${sum}`,
    });
    steps.push({
      description: "Formed equation",
      value: `x + ${multiple}x = ${sum}  =>  x(${multiple} + 1) = ${sum}`,
    });
    steps.push({
      description: "Solved for child's age",
      value: `x = ${sum} / ${multiple + 1} = ${childAge}`,
    });
    return {
      original: input,
      steps,
      finalAnswer: `Child's age = ${Number(childAge.toFixed(2))}, Parent's age = ${Number(parentAge.toFixed(2))}`,
    };
  }

  // Simpler age problem — "A is X years older than B, sum of ages is Y"
  const ageDiffMatch = wordClean.match(
    /is (\d+(?:\.\d+)?) years older than.*?(?:sum|total).*?(?:age|ages).*?(?:is|=)\s*(\d+(?:\.\d+)?)/,
  );
  if (ageDiffMatch) {
    const diff = Number.parseFloat(ageDiffMatch[1]);
    const sum = Number.parseFloat(ageDiffMatch[2]);
    // younger + (younger+diff) = sum => younger = (sum-diff)/2
    const younger = (sum - diff) / 2;
    const older = younger + diff;
    steps.push({
      description: "Identified: Age problem (difference + sum)",
      value: `Let younger = x, older = x + ${diff}, sum = ${sum}`,
    });
    steps.push({
      description: "Formed equation",
      value: `x + (x + ${diff}) = ${sum}  =>  2x = ${sum - diff}`,
    });
    return {
      original: input,
      steps,
      finalAnswer: `Younger = ${Number(younger.toFixed(2))}, Older = ${Number(older.toFixed(2))}`,
    };
  }

  // Time and Work — "A can do a job in X days and B in Y days, together
  // how many days?" (classic pattern: 1/x + 1/y = 1/total)
  const workMatch = wordClean.match(
    /(?:do|complete|finish).*?(?:work|job|task).*?(\d+(?:\.\d+)?)\s*days?.*?(?:and|,).*?(\d+(?:\.\d+)?)\s*days?/,
  );
  if (workMatch) {
    const a = Number.parseFloat(workMatch[1]);
    const b = Number.parseFloat(workMatch[2]);
    const together = (a * b) / (a + b);
    steps.push({
      description: "Identified: Time and Work problem",
      value: `A alone: ${a} days, B alone: ${b} days`,
    });
    steps.push({
      description: "Applied formula: Together = (A × B) / (A + B)",
      value: `Together = (${a} × ${b}) / (${a} + ${b})`,
    });
    return {
      original: input,
      steps,
      finalAnswer: `${Number(together.toFixed(2))} days`,
    };
  }
