...
export default function BasicCalculator() {
  ...
  return (
    <div className="w-full h-full flex-1 flex flex-col">
      {/* Undo / Redo — upper-left, above the display */}
      <div className="flex items-center justify-start gap-2 mb-1.5">
        <button
          type="button"
          onClick={calc.undo}
          disabled={!calc.canUndo}
          className="flex items-center gap-1 text-xs px-2 py-1 rounded-md text-muted-foreground disabled:opacity-30 hover:bg-muted transition-colors"
          aria-label="Undo"
        >
          <Undo2 size={14} /> Undo
        </button>
