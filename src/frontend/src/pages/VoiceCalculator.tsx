...
import { normalizeSpokenMath } from "../lib/normalizeSpokenMath";
...
function FreeAssistant() {
  ...
  const runQuery = (rawText: string) => {
    ...
    setTimeout(() => {
      ...
      // Try again after converting spoken Hindi ("ek aur ek kitna hota
      // hai" -> "1 + 1") — covers the common case of asking simple math
      // out loud in Hindi rather than typing digits/English words.
      const spokenConverted = normalizeSpokenMath(text);
      if (spokenConverted !== text) {
        const spokenMath = parseAndSolve(spokenConverted);
        if (spokenMath) {
          pushAnswer(formatStepByStep(spokenMath, langMode, showFormula), true);
          return;
        }
        const spokenBasic = evaluateMathExpression(spokenConverted);
        if (spokenBasic !== null) {
          const rounded = String(Number(spokenBasic.toFixed(6)));
          pushAnswer(`**${rounded}**`, true);
          return;
        }
      }
