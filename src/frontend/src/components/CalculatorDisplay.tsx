import { Check, ChevronDown, ChevronUp, Copy, Redo2, Share2, Star, Undo2 } from "lucide-react";
...
export function CalculatorDisplay({
  ...
  const expressionScrollRef = useRef<HTMLButtonElement>(null);
  ...
  // Long expressions (e.g. "284+3455+355+35") wrap across multiple lines
  // in a height-capped box — without this, newly typed digits at the end
  // could scroll out of view since nothing kept the box scrolled to the
  // bottom as it grew.
  useEffect(() => {
    if (expressionScrollRef.current) {
      expressionScrollRef.current.scrollTop = expressionScrollRef.current.scrollHeight;
    }
  }, [expression]);
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl p-4 mb-3 min-h-[230px] flex flex-col bg-display-bg border-2 border-display-border ${className}`}
      style={style}
    >
      ...
      {/* Top control bar — matches the reference design: Undo, Redo, Copy,
          Share as labeled buttons together, Star + a compact Copy icon on
          the far right. */}
        ...
        <div className="flex items-center gap-3">
          {onUndo && (
            <button
              ...
              onClick={onUndo}
              disabled={!canUndo}
              className="flex items-center gap-1 text-[13px] text-display-expr/70 disabled:opacity-30 hover:text-display-expr transition-colors"
              aria-label="Undo"
            >
              <Undo2 size={15} /> Undo
            </button>
          ...
          <button
            type="button"
            onClick={handleCopy}
            className="flex items-center gap-1 text-[13px] text-display-expr/70 hover:text-display-expr transition-colors"
        ...
        <div className="flex items-center gap-2 shrink-0">
          {onSaveFavorite && (
            <button
              ...
              className={`p-1 rounded-full transition-colors ${isFavorite ? "text-amber-400" : "text-display-expr/60 hover:text-display-expr"}`}
          ...
          <div className="relative">
            <button
              type="button"
              onClick={handleCopy}
              className="p-1 rounded-full text-display-expr/60 hover:text-display-expr transition-colors"
              aria-label="Copy result"
            >
      ...
      <div className="relative flex-1 flex flex-col justify-end">
        ...
        {/* Expression — tap to bring it back for editing; wraps and
            auto-scrolls to the newest part so long expressions like
            "284+3455+355+35" never have their latest digits hidden. */}
        <button
